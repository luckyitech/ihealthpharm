...
@Component
public class SalePriceValueForCurrentStockExcel extends ReportsExcelUtility {
	...
	private void generateTotalTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle, ReportsMappingModel model,
		...
		double totalSaleValue = responseList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("SALE_VALUE")?String.valueOf(mapper.get("SALE_VALUE")):"0")).sum(); 
		...
		cell.setCellValue("Total Sale Value : ");
		
		cell = dataRow.createCell(10);
		
		cell.setCellValue(totalSaleValue);
	...
	private void createSupplierTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle ,
		...
		if (!ObjectUtils.isEmpty(purchaseMarginList)) {
			...
			cell.setCellValue("SALE DISCOUNT");
			cell.setCellStyle(headerStyle);	
