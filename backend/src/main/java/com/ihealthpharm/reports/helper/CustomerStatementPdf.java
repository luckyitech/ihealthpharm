...
public class CustomerStatementPdf extends ReportsPDFUtility{
	...
	private void generateTotalTable(Document document, ReportsMappingModel model, List<Map<String, Object>> responseList) throws DocumentException {
		...
		PdfPCell nameCell = new PdfPCell(new Phrase("Total Amount Spent"+" "+" : "+"	"+String.format("%.2f",totalAmtPaid), bold)); 
		...
		PdfPCell nameCell2 = new PdfPCell(new Phrase("Total Amount Received"+" "+":"+" "+String.format("%.2f",totalAmtReceived), bold)); 
		...
		PdfPCell nameCell3 = new PdfPCell(new Phrase("Total Outstanding Amount"+" "+":"+" "+String.format("%.2f", totalTotalOutstanding), bold)); 
