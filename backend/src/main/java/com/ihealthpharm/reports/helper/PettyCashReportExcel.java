...
@Component
public class PettyCashReportExcel extends ReportsExcelUtility{
	...
	public void generateReport(List<Map<String, Object>> responseList, ReportsMappingModel model, File responseFile,String inputJson) {
		...
		/*if(!ObjectUtils.isEmpty(salesProductMap)) { 
			...
			for(String productSummary :salesProductMap.keySet()) {	
				...
				createSupplierTable(sheet, responseFile, borderStyle, headerStyle,supplierList, currentRow); 
				generateTotalTable(sheet, responseFile,borderStyle,model,supplierList);
			}
