...
public interface ProviderRepository extends JpaRepository<ProviderModel, Integer>{
	...
	@Query("select p from provider p where (p.firstName like :name% or p.lastName like :name% or p.deaNumber like :name% or p.licenseNumber like :name% ) and p.activeS='Y' ")
