...
@Component
public class DeliveryNotePdf extends ReportsPDFUtility{
...
//		log.info("table width [{}]",table.getTotalWidth());
	...
	private void addTotals(Document document, ReportsMappingModel model, List<Map<String, Object>> responseList) throws DocumentException, ParseException {
		...
		PdfPCell cell = new PdfPCell(new Phrase("Tot. Amt",bold));
		...
		cell = new PdfPCell(new Phrase("Courier Sign",bold));
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		cell.setBorder(Rectangle.NO_BORDER);
		table.addCell(cell);
		...
		cell = new PdfPCell(new Phrase("Customer Sign",bold));
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		cell.setBorder(Rectangle.BOTTOM);
		table.addCell(cell);
