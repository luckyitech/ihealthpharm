...
public class PurchaseInvoiceDetailsExcel extends ReportsExcelUtility {
	...
	private void generateTotalTable(XSSFSheet sheet,File responseFile, CellStyle borderStyle, ReportsMappingModel model,
		...
		double vatTotal = responseList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("VAT")?String.valueOf(mapper.get("VAT")):"0")).sum(); 
