...
@Component
public class SupplierStatementExcel extends ReportsExcelUtility{
...
//}
	...
	private void generateTotalTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle, ReportsMappingModel model,
		...
		double TotalAmtReceived = responseList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("balance")?String.valueOf(mapper.get("balance")):"0")).sum(); 
...
//		cell1=dataRow1.createCell(10);
	...
	private void createSupplierTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle ,
		...
		if (!ObjectUtils.isEmpty(productList)) {
			...
			if(inputJson.get("FROM_APPROVED_DATE")!=null && inputJson.get("TO_APPROVED_DATE")!=null && inputJson.get("SP_NAME")==null) {
			int currentRow = sheet.getLastRowNum();
			
			double totalAmount = productList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("balance")?String.valueOf(mapper.get("balance")):"0")).sum(); 
