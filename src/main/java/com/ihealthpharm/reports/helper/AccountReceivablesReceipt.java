...
public class AccountReceivablesReceipt extends ReportsPDFUtility {
	...
	private void addCreditNoteDetails(Document document, ReportsMappingModel model, List<Map<String, Object>> responseList) throws DocumentException {
		...
		String receiptNo = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("RECEIPT_NO"));
		String customerName = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("CUSTOMER_NAME"));
		String receiptDate = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("RECEIPT_DATE"));
		String billNo = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("BILL_NUMBER"));
		String amount = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("AMOUNT_TO_BE_RECEIVED"));
		...
		cell = new PdfPCell(new Phrase(receiptNo,bold));
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		cell.setColspan(2);
		cell.setBorder(Rectangle.TOP);
		table.addCell(cell);
		...
		cell = new PdfPCell(new Phrase(receiptDate,bold));
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		cell.setColspan(2);
		cell.setBorder(Rectangle.NO_BORDER);
		table.addCell(cell);
		...
		cell = new PdfPCell(new Phrase(billNo,bold));
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		cell.setColspan(2);
		cell.setBorder(Rectangle.NO_BORDER);
		table.addCell(cell);
