...
public class OutstandingReceivablesByAccountPdf extends ReportsPDFUtility{
	...
	private void generateTotalTable(Document document, ReportsMappingModel model, List<Map<String, Object>> responseList) throws DocumentException {
		...
		double totalAmount;
		totalAmount = responseList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("AMOUNT_TO_BE_RECEIVED")?String.valueOf(mapper.get("AMOUNT_TO_BE_RECEIVED")):"0")).sum(); 
		...
		PdfPCell nameCell2 = new PdfPCell(new Phrase("Outstanding Amount  : "+amount+"   ", title08)); 
	...
	private void createTable(Document document, ReportsMappingModel model, List<Map<String, Object>> productList,
		...
		//for (HeaderDto hearder : headerList) {
			...
			headerCell = new Paragraph();
			headerCell.setFont(headerFont);
			headerCell.add("CREDIT DAYS LEFT");
			cell = new PdfPCell(headerCell);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			if (!model.isShowVerticalLines())
				cell.setBorder(Rectangle.BOTTOM);
			...
			headerCell = new Paragraph();
			headerCell.setFont(headerFont);
			headerCell.add("CREDIT LIMIT LEFT");
			cell = new PdfPCell(headerCell);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			if (!model.isShowVerticalLines())
				cell.setBorder(Rectangle.BOTTOM);
		...
		if (!ObjectUtils.isEmpty(productList)) {
			...
			totalAmount = productList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("AMOUNT_TO_BE_RECEIVED")?String.valueOf(mapper.get("AMOUNT_TO_BE_RECEIVED")):"0")).sum(); 
