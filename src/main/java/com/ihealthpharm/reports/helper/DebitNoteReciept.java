...
public class DebitNoteReciept extends ReportsPDFUtility {
	...
	private void addCreditNoteDetails(Document document, ReportsMappingModel model, List<Map<String, Object>> responseList) throws DocumentException {
		...
		String debitNoteNo = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("debit_note_no"));
		String supplierName = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("SP_NAME"));
		String debitNoteDate = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("debit_date"));
		String purchaseInvoiceNo = (ObjectUtils.isEmpty(responseList))?"":String.valueOf(responseList.get(0).get("invoice_id"));
