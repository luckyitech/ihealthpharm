...
@Component
	...
	public void generateReport(List<Map<String, Object>> responseList, ReportsMappingModel model, File responseFile) {
		...
		Map<String, List<Map<String, Object>>> purchaseDetailsMap = responseList.stream()
				.collect(Collectors.groupingBy(map -> (String) map.get("BATCH_NO")));			
		...
		if(!ObjectUtils.isEmpty(purchaseDetailsMap)) { 
			...
			for(String batchNo :purchaseDetailsMap.keySet()) {	
				int currentRow = sheet.getLastRowNum();
				List<Map<String, Object>> purchaseDetailsList = purchaseDetailsMap.get(batchNo);
				createSupplierTable(sheet, responseFile, borderStyle, headerStyle,purchaseDetailsList, batchNo, currentRow); 
			}
	...
	private void createSupplierTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle ,
			CellStyle headerStyle, List<Map<String, Object>> purchaseDetailsList, String batchNo, int rowNum) {
		...
		if (!ObjectUtils.isEmpty(purchaseDetailsList)) {
			...
			for (Map<String, Object> rowData : purchaseDetailsList) {
				...
				Object value = rowData.containsKey("BATCH_NO") ? rowData.get("BATCH_NO") : "";
				headCell.setCellValue("Batch No  :   ");
