...
//		String billDate="";
		...
		System.out.println(dataMap.get("FROM_SYSTEM_DATE"));

			
			PdfPTable supllierNameTable = new PdfPTable(3);
			PdfPCell nameCell = new PdfPCell(new Phrase("From Date : "+String.valueOf((dataMap.get("FROM_SYSTEM_DATE")))+"          "+"To Date   : "+String.valueOf(dataMap.get("TO_SYSTEM_DATE")), title08)); 
		...
		//for (HeaderDto hearder : headerList) {
			...
			headerCell = new Paragraph();
			headerCell.setFont(headerFont);
			headerCell.add("LAST UPDATED USER");
			cell = new PdfPCell(headerCell);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			if (!model.isShowVerticalLines())
				cell.setBorder(Rectangle.BOTTOM);
			...
			headerCell = new Paragraph();
			headerCell.setFont(headerFont);
			headerCell.add("LAST UPDATED DT");
			cell = new PdfPCell(headerCell);
			cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			if (!model.isShowVerticalLines())
				cell.setBorder(Rectangle.BOTTOM);
