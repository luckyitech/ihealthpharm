...
@Component
public class PurchaseDetailsByBatchNo extends ReportsPDFUtility {
	...
	public Document generateReport(List<Map<String, Object>> responseList, ReportsMappingModel model,
		...
		try {
			...
			if(!ObjectUtils.isEmpty(purchaseOrderDetails)) { 
				...
				for(String batchNo :purchaseOrderDetails.keySet()) {	
					List<Map<String, Object>> purchaseOrderDetailsMap = purchaseOrderDetails.get(batchNo);
					createTable(document,model,purchaseOrderDetailsMap,batchNo);
	...
	private void createTable(Document document, ReportsMappingModel model, List<Map<String, Object>> purchaseOrderDetailsList,
			String batchNo) throws DocumentException {
		...
		PdfPCell nameCell = new PdfPCell(new Phrase("Batch No  : "+batchNo,title08)); 
