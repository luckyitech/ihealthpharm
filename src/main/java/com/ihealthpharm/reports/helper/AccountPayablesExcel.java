...
public class AccountPayablesExcel extends ReportsExcelUtility{
	...
	private void generateTotalTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle, ReportsMappingModel model,
		...
		double TotalAmtPaid = responseList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("TOTAL_AMOUNT_PAID")?String.valueOf(mapper.get("TOTAL_AMOUNT_PAID")):"0")).sum(); 
		double TotalAmtToBePaid = responseList.stream().mapToDouble(mapper->Double.parseDouble(mapper.containsKey("TOTAL_AMOUNT_TO_BE_PAID")?String.valueOf(mapper.get("TOTAL_AMOUNT_TO_BE_PAID")):"0")).sum(); 
