...
@Component
public class PrintQuotationReceiptExcel extends ReportsExcelUtility{
	...
	public void generateReport(List<Map<String, Object>> responseList, ReportsMappingModel model, File responseFile,String inputJson) {
		...
		if(!ObjectUtils.isEmpty(QuotationDetailsMap)) { 
			...
			for(String itemName :QuotationDetailsMap.keySet()) {	
				...
				List<Map<String, Object>> quotationDetails = QuotationDetailsMap.get(itemName);
				createQuotationTable(sheet, responseFile, borderStyle, subHeaderStyle,headerStyle,quotationDetails, itemName, currentRow);
	...
	private void createQuotationTable(SXSSFSheet sheet,File responseFile, CellStyle borderStyle ,
			CellStyle subHeaderStyle,CellStyle headerStyle, List<Map<String, Object>> quotationDetails, String itemName, int rowNum) {
		...
		if (!ObjectUtils.isEmpty(quotationDetails)) {
			...
			Object value = quotationDetails.get(0).containsKey("QUOTATION_NO") ? quotationDetails.get(0).get("QUOTATION_NO") : "";
			headCell.setCellValue("Quotation No  :   ");
			...
			value = quotationDetails.get(0).containsKey("SP_NAME") ? quotationDetails.get(0).get("SP_NAME") : "";
			...
			remarksCell1.setCellValue("REMARKS :");
	...
	public int setHeader(SXSSFWorkbook workbook, SXSSFSheet sheet, ReportsMappingModel model,CellStyle headerStyle) { 
		...
		wrapStyleLeft.setAlignment(HorizontalAlignment.LEFT);
	...
	public String getReportHeader(HeaderFooterContentDto contentDto) {
		...
		if (!ObjectUtils.isEmpty(contentDto)) {
			...
			buffer.append("Request For Quotation");
