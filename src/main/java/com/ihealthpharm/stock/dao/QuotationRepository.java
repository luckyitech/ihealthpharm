...
//	List<QuotationModel> getPendingQuoationsByStatus(@Param("status") String status);
	...
	List<ItemSupplierDTO> getItemsBySupplierAndScannedCode(@Param("supplierId")Integer supplierId, @Param("scanCode")String scanCode);
	  ...
	  @Query("select new com.ihealthpharm.masters.dto.ItemSupplierDTO(id.itemSupplierId,id.activeS,d.name as supplierName,i.itemName,m.name as manufacturerName,m.licence as manufacturerLicense,i.itemDescription,i.itemId,d.supplierId,id.supplierPriority,f.form as formulation,i.itemCode,id.unitRate,id.discountPercentage,id.validity,i as itemsModel) from items_supplier id inner join supplier d on id.suppliersId=d.supplierId inner join items i on i.itemId=id.itemsId inner join items_forms f on i.itemForm=f.itemformId " + 
				" inner join manufacturer m on m.manufacturerId=i.manufacturer where "
				+ "  i.barcode like %:barcode%  and id.activeS='Y'")
	List<ItemSupplierDTO> getItemsByItemBarcodeForQuotation(@Param("barcode")String barcode);
	...
	List<ItemSupplierDTO> getItemsByBarcodeAndSupplier(@Param("barCode")String barCode, @Param("supplierId")Integer supplierId);
	  ...
	  @Query("select new com.ihealthpharm.masters.dto.ItemSupplierDTO(a.itemSupplierId,d.name as supplierName,i.itemName,m.name as manufacturerName,m.licence as manufacturerLicense,i.itemDescription,i.itemId,d.supplierId,a.supplierPriority,f.form as formulation,i.itemCode, "
	  		...
	  		+ "i as itemsModel,i.reOrderQuantity,a.autoQuotId,a.activeS,a.bonus,a.barcode) from  auto_quotations a "
	  ...
	  @Query("select new com.ihealthpharm.masters.dto.ItemSupplierDTO(a.itemSupplierId,d.name as supplierName, "
	  		...
	  		+ "a.discountPercentage,a.validity,i as itemsModel,i.reOrderQuantity,a.autoQuotId,a.activeS,a.bonus,a.barcode) from auto_quotations a inner join supplier d on "
	  		+ "a.supplierId=d.supplierId "
	  		+ "inner join items i on i.itemId=a.itemId "
	  		+ "inner join items_forms f on i.itemForm=f.itemformId " + 
				" inner join manufacturer m on m.manufacturerId=i.manufacturer where "
				+ " i.itemCode like %:itemCode%  or i.itemName like %:itemName% or i.itemDescription like :itemDescription% or a.barcode like %:barcode%")
		List<ItemSupplierDTO> getItemsByItemCodeOrItemNameorItemDescForAutoQuotation(@Param("itemCode") String itemCode, @Param("itemName") String itemName, 
				@Param("itemDescription") String itemDescription,@Param("barcode") String barcode);
}
